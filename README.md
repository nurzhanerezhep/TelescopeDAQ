...
GUI показывает модель подключённого АЦП и номер run, число событий по задаваемым
временным интервалам за последние 30 минут и обновляемый спектр максимальной
амплитуды каждого события. Сбор выполняется
в отдельном потоке и безопасно останавливается кнопкой `Стоп` или при закрытии
окна. В памяти остаются последние 30 минут временной статистики и не более
100 000 значений спектра.
